...
class PaymentMethodDimensionService:
    ...
    def update_payment_method_dimension(self, payment: Payment) -> None:
        try:
            dim_payment_method = self.session.query(DimPaymentMethod)\
                .filter(DimPaymentMethod.payment_method_id == payment.payment_method_id)\
                .first()
            ...
            if not dim_payment_method:
                dim_payment_method = DimPaymentMethod(
                    ...
                    is_digital=payment.payment_method_type in [1, 2],  # Example mapping
