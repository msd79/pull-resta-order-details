# File location: src/services/customer_dimension.py
...
class CustomerDimensionService:
    ...
    def update_customer_dimension(self, customer: Customer) -> None:
        ...
        try:
            ...
            # Get current metrics
            self.logger.debug("Calculating customer metrics...")
            metrics = self.get_customer_metrics(customer.id)
            self.logger.debug(f"Customer metrics calculated: {metrics}")
            
            # Get current customer dimension record
            self.logger.debug("Checking for existing customer dimension record...")
            ...
            self.logger.debug("Transforming customer data...")
            new_record = self.transform_customer(customer, metrics)
            
            if current_record:
                self.logger.debug("Found existing customer record, checking for changes...")
                # Check if any tracked attributes have changed
                if self._has_tracked_changes(current_record, new_record):
                    self.logger.info("Detected changes in tracked attributes. Creating new version...")
                    ...
                    self.logger.debug("No tracked changes detected. Updating non-tracked attributes...")
                    # Update non-tracked attributes of current record
                    self._update_non_tracked_attributes(current_record, new_record)
            else:
                self.logger.info("No existing record found. Creating first version...")
